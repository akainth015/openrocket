...
public class Transformation implements java.io.Serializable {
     ...
     *
     * @param rotation    matrix for an affine transformation
     * @param translation the offset to apply to inputs after the rotation
     */
    ...
    public static Transformation rotate_about_vertical_axis(double theta, Coordinate axisXZ) {
        if (ANGLE_EPSILON > Math.abs(theta)) {
            return Transformation.IDENTITY;
        }
        ...
        return new Transformation(
                new double[][]{
                        {Math.cos(theta), axisXZ.x, Math.sin(theta)},
                        {0, 1, 0},
                        {-Math.sin(theta), axisXZ.z, Math.cos(theta)}
                }
        );
    }
